...
open_from_update_dir (MonoAssemblyName *aname, char **assemblies_path, void *user_data)
{
	...
	if (inMemoryAssemblies.has_assemblies ()) {
		...
		if (result != nullptr) {
			log_info (LOG_DEFAULT, "Loaded assembly %s from memory in domain %d", mono_assembly_name_get_name (aname), mono_domain_get_id (domain));
			return result;
		}
		log_info (LOG_DEFAULT, "No in-memory data found for assembly %s", mono_assembly_name_get_name (aname));
	} else {
		log_info (LOG_DEFAULT, "No in-memory assemblies detected", mono_assembly_name_get_name (aname));
	}
#endif
...
load_assembly (MonoDomain *domain, JNIEnv *env, jstring_wrapper &assembly)
{
	...
	if (inMemoryAssemblies.has_assemblies () && inMemoryAssemblies.load_assembly_from_memory (domain, aname) != nullptr) {
		log_info (LOG_DEFAULT, "Dynamically opened assembly %s from memory", mono_assembly_name_get_name (aname));
	} else
#endif
